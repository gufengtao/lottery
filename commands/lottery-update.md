---
description: 更新双色球历史开奖数据 - 从中国福彩网官方同步数据，追加新期数并保留所有历史记录
allowed-tools:
  - Bash
---

# Lottery Update Command - 更新历史开奖数据

从官方彩票网站爬取双色球开奖数据，追加新期数并保留所有历史记录。

## 数据来源

- **网站**: 中国福彩网官方 (kaijiang.zhcw.com)
- **保存路径**: `./data/lottery-history.json` (插件目录内)
...
if os.path.exists(output_file):
    try:
        ...
        print(f'📂 已加载 {len(existing_history)} 期历史数据')
    except:
        print('⚠️ 无法加载现有数据，将创建新文件')
...
print(f'\n✅ 已保存到：{output_file}')
print(f'📊 本期新增 {len(merged_history) - len(existing_history)} 期')
print(f'📊 总计 {len(merged_history)} 期历史数据（追加模式，不删除旧数据）')

if merged_history:
    latest = merged_history[0]
    print(f'\n📌 最新期号：{latest["issue"]} ({latest["date"]})')
    print(f'   红球：{" ".join(latest["red_balls"])}')
    print(f'   蓝球：{latest["blue_ball"]}')

    if len(merged_history) > 1:
        earliest = merged_history[-1]
        print(f'\n📌 最早期号：{earliest["issue"]} ({earliest["date"]})')
```
